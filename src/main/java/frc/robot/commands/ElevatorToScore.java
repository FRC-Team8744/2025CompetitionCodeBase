...
public class ElevatorToScore extends Command {
  ...
  public void execute() {
    ...
    if (m_robotDrive.autoRotateSpeed == 0) {
      ...
      if (m_elevator.getMotorPosition() >= ((16.35 * Constants.ELEVATOR_GEARING * m_elevator.percentOfElevator) * .50) && toggle) {
        m_robotDrive.isAutoYSpeed = true;
        m_robotDrive.isAutoXSpeed = true;
        m_scoringMechPivot.rotatePivot(m_scoringMechPivot.scoringMechGoalAngle);
        toggle = false;
      }
    }
  }
